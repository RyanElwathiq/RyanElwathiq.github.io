// ═══════════════════════════════════════════════════════════════
//  فيلم الأرقام — النسخة العمودية 9:16 (مولّدة آلياً من promofilm2)
//
...
//
//  التشغيل: node _check/promofilm2v.mjs numbers ar|en [preview]
...
const glow = (o = 1) =>
  `<div style="position:absolute;inset:0;background:radial-gradient(70% 30% at ${isAr ? '88%' : '12%'} 6%, ${ACCENT}${Math.max(
    0,
...
const ending = (t, at) => {
  ...
  </div>${thread(w(t, at + 1.5, at + 7), 1680, 0.55)}`;
};
...
const numbers = (t) => {
  ...
  if (t < 8.69) {
    ...
    s = `${glow(0.4)}<div class="center" style="gap:52px">
      <h1 style="font-size:52px;font-weight:700;color:${MUTED};opacity:${a1}">${T.s1a}</h1>
    ...
    const rowsH = T.s2
      .map(([n, label], i) => {
        ...
        return `<div style="display:flex;align-items:baseline;gap:36px;opacity:${a};transform:translateY(${(1 - a) * 26}px)">
          <span class="num" style="font-size:96px;line-height:1">${count(n, p)}</span>
          <span style="font-size:34px;line-height:1.42;font-weight:700">${label}</span></div>`;
      })
      .join('');
    s = `${glow()}<div class="center" style="gap:54px;align-items:flex-start;padding:0 80px">
      ...
      .map(([n, label], i) => {
        ...
        return `<div style="width:100%;opacity:${a}">
          <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:12px">
            <span style="font-size:32px;line-height:1.42;font-weight:700">${label}</span>
            <span class="num" style="font-size:64px;line-height:1">${count(n, p)}%</span></div>
    ...
    s = `${glow()}<div class="center" style="gap:44px;padding:0 90px">
      <p style="font-size:50px;color:${MUTED};opacity:${a0}">${T.s3t}</p>
      ${bars}
      <h1 style="font-size:38px;font-weight:800;color:${ACCENT};margin-top:8px;opacity:${c}">${T.s3c}</h1>
    </div>`;
  } else if (t < 46.43) {
    ...
    s = `${glow()}<div class="center" style="gap:54px">
      <h1 style="font-size:68px;font-weight:800;opacity:${a1}">${T.s4a}</h1>
      <h1 style="font-size:68px;font-weight:800;color:${ACCENT};opacity:${a2}">${T.s4b}</h1>
    ...
    s = `${glow(0.7)}<div class="center" style="gap:56px">
      <h1 style="font-size:50px;font-weight:700;color:${MUTED};opacity:${a1}">${T.s5a}</h1>
      <h1 style="font-size:56px;font-weight:800;opacity:${a2}">${T.s5b}</h1>
    </div>`;
  } else if (t < 72.04) {
    ...
    const chips = T.s6
      ...
      .join('');
    s = `${glow()}<div class="center" style="gap:46px;align-items:flex-start;padding:0 100px">
    ...
    s = `${glow()}<div class="center" style="gap:54px">
      <h1 style="font-size:54px;font-weight:800;opacity:${a1}">${T.s7a}</h1>
      <h1 style="font-size:54px;font-weight:800;color:${ACCENT};opacity:${a2}">${T.s7b}</h1>
    ...
    s = `${glow(0.9)}<div class="center" style="gap:48px">
      <h1 style="font-size:58px;font-weight:800;opacity:${a1}">${T.s8a}</h1>
      <div style="background:#151A0E;border:2px solid ${ACCENT}88;border-radius:22px;padding:26px 52px;
           font-size:36px;line-height:1.55;font-weight:700;opacity:${a2};transform:translateY(${(1 - a2) * 24}px);
...
if (!pick) {
  console.log('حدد: node _check/promofilm2v.mjs numbers ar|en [preview]');
  process.exit(1);
}
