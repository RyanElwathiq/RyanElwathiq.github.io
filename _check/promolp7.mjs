// ═══════════════════════════════════════════════════════════════
//  فيلم مقال «أول إعلان ممول» (2026-08-07)
//
...
//
//  التشغيل: node _check/promolp7.mjs firstad ar|en [preview]
...
const ending = (t, at) => {
  ...
  return `${glow(1.4)}<div class="center" style="gap:48px">
    <img src="${logo}" style="width:190px;height:190px;opacity:${lg};transform:scale(${0.74 + 0.26 * lg});
...
const firstad = (t) => {
  ...
  if (t < 12) {
    ...
    s = `${glow(0.32)}<div class="center" style="gap:54px">
      <h1 style="font-size:80px;font-weight:800;opacity:${a1};transform:translateY(${(1 - a1) * 26}px)">${T.s1a}</h1>
      <h1 style="font-size:60px;font-weight:700;color:${MUTED};opacity:${a2};transform:translateY(${(1 - a2) * 22}px)">${T.s1b}</h1>
    </div>`;
  } else if (t < 28) {
    // S2 — الفرق الجوهري
    ...
    s = `${glow(0.6)}<div class="center" style="gap:46px">
      <h1 style="font-size:88px;font-weight:800;opacity:${a1};transform:scale(${1.06 - 0.06 * a1})">${T.s2a}</h1>
      <h1 style="font-size:54px;font-weight:700;color:${MUTED};opacity:${a2}">${T.s2b}</h1>
      <h1 style="font-size:60px;font-weight:800;color:${ACCENT};opacity:${a3}">${T.s2c}</h1>
    </div>`;
  } else if (t < 46) {
    // S3 — العدّاد بلا إبرة (نفس مجرّد التصميم، متحرك)
    const a1 = easeOut(w(t, 28, 29.4));
    const a2 = easeOut(w(t, 33, 34.6));
    s = `${glow(0.8)}${artGauge(w(t, 30, 43), W * 0.5, H * 0.74, 210)}
      <div style="position:absolute;top:120px;left:0;width:100%;text-align:center;padding:0 200px">
        <h1 style="font-size:74px;font-weight:800;opacity:${a1}">${T.s3a}</h1>
        <h1 style="font-size:56px;font-weight:700;color:${ACCENT};margin-top:26px;opacity:${a2}">${T.s3b}</h1>
      </div>`;
  } else if (t < 70) {
    ...
    const chips = T.s4
      ...
      .join('');
    s = `${glow(1)}<div class="center" style="gap:40px;align-items:flex-start;padding:0 440px">
      <p style="align-self:center;font-size:48px;color:${MUTED};opacity:${a0};margin-bottom:12px">${T.s4t}</p>
      ${chips}
    </div>`;
  } else if (t < 86) {
    // S5 — الهدف الغلط
    ...
    s = `${glow(0.9)}${artTarget(w(t, 71, 84), W * 0.5, H * 0.68)}
      <div style="position:absolute;top:110px;left:0;width:100%;text-align:center;padding:0 200px">
        <h1 style="font-size:78px;font-weight:800;color:${ACCENT};opacity:${a1}">${T.s5a}</h1>
        <h1 style="font-size:58px;font-weight:700;margin-top:24px;opacity:${a2}">${T.s5b}</h1>
      </div>`;
  } else if (t < 100) {
    // S6 — التعلّم بيرجع للصفر
    const a1 = easeOut(w(t, 86, 87.4));
    const a2 = easeOut(w(t, 91, 92.6));
    s = `${glow(0.7)}${artSaw(w(t, 87, 99), H * 0.86)}
      <div style="position:absolute;top:130px;left:0;width:100%;text-align:center;padding:0 200px">
        <h1 style="font-size:70px;font-weight:800;opacity:${a1}">${T.s6a}</h1>
        <h1 style="font-size:66px;font-weight:800;color:${ACCENT};margin-top:24px;opacity:${a2}">${T.s6b}</h1>
      </div>`;
  } else if (t < 113) {
    // S7 — الغربال: رقم واحد بيقرر
    const a1 = easeOut(w(t, 100, 101.4));
    const a2 = easeOut(w(t, 104, 105.6));
    s = `${glow(0.9)}${artSieve(w(t, 100.5, 112), W * 0.5, H * 0.40, H * 0.80)}
      <div style="position:absolute;top:110px;left:0;width:100%;text-align:center;padding:0 200px">
        <h1 style="font-size:66px;font-weight:700;color:${MUTED};opacity:${a1}">${T.s7a}</h1>
        <h1 style="font-size:80px;font-weight:800;color:${ACCENT};margin-top:20px;opacity:${a2}">${T.s7b}</h1>
      </div>`;
  } else if (t < 121) {
    // S8 — الدعوة
    ...
    s = `${glow(1.1)}<div class="center" style="gap:50px">
      ...
      <div style="background:#151A0E;border:2px solid ${ACCENT}88;border-radius:22px;padding:28px 56px;
           font-size:46px;font-weight:700;opacity:${a2};transform:translateY(${(1 - a2) * 24}px);
...
if (process.argv[4] === 'preview') {
  ...
  for (const pt of [9, 24, 40, 62, 82, 97, 110, 118, 124]) {
    ...
    await page.screenshot({ path: `${PV}/fa-${lang}-${pt}.png` });
  }
  await browser.close();
