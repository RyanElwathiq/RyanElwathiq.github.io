// أغلفة موقعية لفيديوهي SEO والمونتاج (عربي + إنجليزي) — نفس النمط الموقعي
...
const site169 = (isAr, h1, h2) => `<!doctype html><html dir="${isAr ? 'rtl' : 'ltr'}"><head><meta charset="utf-8"><style>
  ...
  *{margin:0;box-sizing:border-box}
  ...
  <div style="position:absolute;top:150px;left:0;width:100%;text-align:center;padding:0 110px">
    <h1 style="font-size:56px;font-weight:800;color:${MUTED};text-shadow:0 4px 40px rgba(0,0,0,.9)">${h1}</h1>
...
const COVERS = [
  ['lp-seo-cover', site169(true, 'بتكتب اسم شغلك بجوجل…', 'وبتلاقي منافسك. مش إنت.')],
  ['lp-seo-en-cover', site169(false, 'Type your business into Google…', 'and find your competitor. Not you.')],
  ['lp-montage-cover', site169(true, 'فيديوهاتك حلوة.', 'بس ما حدا بيكمّلها.')],
  ['lp-montage-en-cover', site169(false, 'Your videos look good.', 'But nobody finishes them.')],
];
...
for (const [name, html] of COVERS) {
  ...
  await page.screenshot({ path: `${SITE}/${name}.jpg`, type: 'jpeg', quality: 88 });
  console.log('✅', name);
}
await browser.close();
