...
export default {
  ...
  // ═══════════════════════════════════════════════════════════
  //  الكرون — التقرير الصباحي + تنبيهات بلوكات الخطة
  //
  //  ⚠️ كلاودفلير بيسمح بخمس كرونات كحد أقصى للـ Worker الواحد.
  //     ولهيك التفريع بالكود مش بالكرون: كرون لكل ساعة تنفيذ،
  //     والجمعة بتنفلتر جوّا الدالة.
  //
  //  ⚠️ الأردن ألغى التوقيت الصيفي من ٢٠٢٢ — UTC+3 طول السنة.
  //     يعني الحساب ثابت ولا بيحتاج تعديل بالشتا.
  //     UTC 05:00 = ٨:٠٠ عمّان · 08:00 = ١١:٠٠ · 10:00 = ١٣:٠٠ · 18:00 = ٢١:٠٠
  //
  //  ⚠️ بكل الأوقات فوق، يوم UTC = يوم عمّان (أقصى إزاحة ٢١:٠٠+٣ = نفس اليوم)
  //     فـ getUTCDay() بتكفي، وما بدها تحويل منطقة زمنية.
  // ═══════════════════════════════════════════════════════════
  async scheduled(event, env, ctx) {
    ctx.waitUntil(
      (async () => {
        ...
        const isFriday = new Date().getUTCDay() === 5; // الجمعة عطلة بالخطة

        // ── ٨:٠٠ — التقرير الصباحي (كل يوم) ──
        if (cron === '0 5 * * *') {
          await tgSend(env, await buildDigest(env, true));
          if (isFriday) {
            await tgSend(
              env,
              '🧭 <b>الجمعة — يوم تجهيز</b>\n\n' +
                'ولا رسالة صيد اليوم. اللي بينعمل:\n' +
                '• تجهيز أهداف الأسبوع الجاي\n' +
                '• مراجعة الردود اللي تجمّعت\n' +
                '• ستوري خفيف وبس\n\n' +
                '<i>الراحة جزء من الخطة مش خروج عنها.</i>',
            );
          } else {
            await tgSend(
              env,
              '🎯 <b>٩:٠٠ — بلوك الصيد</b>\n\n' +
                '<b>٩ أهداف</b> · <code>ملف الصيد — نسخة ٢ برسالتين.pdf</code>\n' +
                'رسالة للحاجب بتسأل مين المسؤول، ورسالة لصاحب القرار.\n\n' +
                '🔴 <b>افحص إنستغرام قبل كل رسالة واكتب المتابعين.</b>\n' +
                'اثنان من التسعة عندهم مئات الآلاف من المتابعين، و«ما حدا بيلاقيك» ' +
                'كانت رح تحرقهم. الأسماء بالملف مش هون, الريبو عام.\n\n' +
                '⚠️ المحاماة مستبعدة نهائياً (م.٦٠ نقابة المحامين).\n' +
                '⚠️ ولا معلومة عرفتها عنه بتترد عليه كنقد.',
            );
          }
          return;
        }

        if (isFriday) return; // باقي البلوكات بتوقف يوم الجمعة

        // ── ١١:٠٠ — الوظائف ──
        if (cron === '0 8 * * *') {
          await tgSend(
            env,
            '💼 <b>١١:٠٠ — بلوك الوظائف</b>\n\n' +
              '<b>٥ تقديمات + رسالتان</b>\n' +
              '• أ — تفصيل كامل ×٢ (~١٥د)\n' +
              '• ب — سريعة ×٣ (~٥د)\n' +
              '• ج — رسالتان مباشرتان على لينكدإن\n\n' +
              'الدفعة الجاهزة: <code>Outreach/دفعة-السبت-وظائف.md</code>\n\n' +
              '🛑 عند التقديم ٤٠ وصفر ردود — وقّف وراجع الملف مش العدد.',
          );
          return;
        }
        ...
        if (cron === '0 10 * * *') {
          await tgSend(
            env,
            '📱 <b>١٣:٠٠ — بلوك التفاعل</b>\n\n' +
              '✅ بوست اليوم <b>مجدول ومنشور لحاله</b> — ما بده شغل.\n\n' +
              'اللي بينعمل بهالبلوك:\n' +
              '• <b>١٠ تعليقات ذكية</b> على بوستات أصحاب مشاريع\n' +
              '• رد على تعليقات بوست امبارح\n' +
              '• شيك إذا حدا سأل بالخاص\n\n' +
              '⚠️ بوست واحد باليوم لكل حساب. البروفايل والصفحة حسابين منفصلين.\n' +
              '⚠️ ولا تنشر يدوي فوق المجدول — بتخنق الاثنين.',
          );
          return;
        }
        ...
        // ── ٢١:٠٠ — قراءة الأرقام ──
        if (cron === '0 18 * * *') {
          await tgSend(env, await buildDigest(env, false));
          await tgSend(
            env,
            '📊 <b>٢١:٠٠ — قراءة الأرقام</b>\n\n' +
              '• مين رد اليوم؟ وشو قال بالضبط؟\n' +
              '• أي قناة جابت محادثة جدية؟\n' +
              '• شو بدنا نغيّر بكرا؟\n\n' +
              '<i>كل رد بيتسجّل — بيانات سوق، مش بس متابعة.</i>\n\n' +
              '🎯 الهدف: $400 قبل 2026-08-16',
          );
          return;
        }
      })(),
