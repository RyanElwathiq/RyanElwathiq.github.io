...
function init() {
  ...
  initSequences().then((fns) => {
    // المسار الحي للموبايل بيرجّع دوال تنظيف — منسجّلها عشان
    // تنشال مستمعاته قبل الانتقال لصفحة ثانية
    (fns || []).forEach((f) => typeof f === 'function' && cleanups.push(f));
    ScrollTrigger.sort();
    ScrollTrigger.refresh();
